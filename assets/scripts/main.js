...
async function getRecipes() {
  ...
  //   receiptList.push(localStorage[i]);
  // }
  ...
  return new Promise(async function (resolve, reject) {
      ...
      for (let i = 0; i < RECIPE_URLS.length; i++) {
          ...
          try {
              ...
              const urlList = await fetch(RECIPE_URLS[i]);
              ...
              //            "await" again
              const urlFinal = await urlList.json();
              // A8. TODO - Add the new recipe to the recipes array
              receiptList.push(urlFinal);
              ...
              //            resolve() method.
              if (RECIPE_URLS.length == receiptList.length) {
                  saveRecipesToStorage(receiptList);
                  resolve(receiptList);
              }
          } catch (error) {
